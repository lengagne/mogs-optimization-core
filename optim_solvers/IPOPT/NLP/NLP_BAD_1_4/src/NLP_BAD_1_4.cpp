...
#include "AbstractBAD_1_4Critere.hpp"

using namespace Ipopt;

/* Constructor. */
NLP_BAD_1_4::NLP_BAD_1_4 ()
{
    std::cout<<"Constructor of NLP_BAD_1_4"<<std::endl;
}
...
void NLP_BAD_1_4::load_xml(QDomElement criteres)
{
    ...
    for (QDomElement critere = criteres.firstChildElement ("critere"); !critere.isNull();critere = critere.nextSiblingElement("critere"))
	{
		...
		if (criteres.tagName()=="criteres")
		{
			...
			create_BAD_1_4Critere* creator_;
			destroy_BAD_1_4Critere* destructor_;
	
			if ( mpc.get_library_plugin("MogsCriteriaNlpBAD_1_4",type,library_so))
			{
				// load the library
				...
				// load the symbols
				creator_ = (create_BAD_1_4Critere*) dlsym(library, "create");
				destructor_ = (destroy_BAD_1_4Critere*) dlsym(library, "destroy");
				...
				AbstractBAD_1_4Critere* crit = creator_(critere,&kin);
				// FIXME for the moment no init from the xml
// 				crit->init(critere);	
				std::cout << "name "   <<name.toStdString().c_str() << std::endl;
// 				criteres_.push_back(new CameraBAD_1_4Critere(critere,&kin));
				criteres_.push_back(crit);
			}
			else
			{
				qDebug()<<"Error cannot load the plugin "<<type<<" as an MogsCriteriaNlpBAD_1_4 plugin";
				exit(0);
			}	
		}
	}
...
bool NLP_BAD_1_4::get_nlp_info (Index & n, Index & m, Index & nnz_jac_g,
...
bool NLP_BAD_1_4::get_bounds_info (Index n, Number * x_l, Number * x_u,
...
bool NLP_BAD_1_4::get_starting_point (Index n, bool init_x, Number * x,
...
bool NLP_BAD_1_4::eval_f (Index n, const Number * x, bool new_x, Number & obj_value)
{
...
bool NLP_BAD_1_4::eval_grad_f (Index n, const Number * x, bool new_x, Number * grad_f)
{
	// return the gradient of the objective function grad_{x} f(x)

    assert(n == kin.getNDof());
	B<Number>* X = new B<Number>[n];
	...
	{
		B<Number> tmp = criteres_[j]->compute(X,&akin,&mem_kin);
		tmp.diff(0,n);
		std::cout<<" dtmp/dx0 = "<< X[0].d(0);
 		out = out + tmp;
	}
	out.diff(0,n);
    for(unsigned int i=0;i<n;i++)
...
bool NLP_BAD_1_4::eval_g (Index n, const Number * x, bool new_x, Index m, Number * g)
{
            assert(n == kin.getNDof());
            assert(m == 0);
...
bool NLP_BAD_1_4::eval_jac_g (Index n, const Number * x, bool new_x,
...
bool NLP_BAD_1_4::eval_h (Index n, const Number * x, bool new_x,
...
void NLP_BAD_1_4::finalize_solution (SolverReturn status,
...
extern "C" NLP_BAD_1_4* create()
{
    return new NLP_BAD_1_4();
}

extern "C" void destroy(NLP_BAD_1_4* p)
{
    delete p;
}
